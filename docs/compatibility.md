...
it as a hole. AASL0009 and AASL0006 rename fixes are withheld for names containing
whitespace to preserve Serilog's logged output; the diagnostics still report.
For `{Hello World} {Id}`, Serilog's runtime parser returns only
...
| [AASL0006](rules/AASL0006.md) | Rename duplicate holes to unique names | Invocations only. Subsequent holes are uniquified (`{Test}` `{Test2}`), or renamed from argument identifiers when those can be derived. Leaf and qualified primary suggestions share one used-name set. Qualified names are a second action when they differ. Not offered for `[LoggerMessage]` (renaming a hole would not add a C# parameter). Holes whose names contain whitespace stay diagnostic-only because Serilog treats them as literal text and renaming them would change the logged output. |
...
| [AASL0009](rules/AASL0009.md) | Rename hole to suggested name | Full when the source map is rewriteable; also `[LoggerMessage]` attribute strings. Unmappable and shared-constant templates remain diagnostic-only. Holes whose names contain whitespace stay diagnostic-only because Serilog treats them as literal text and renaming them would change the logged output. |
