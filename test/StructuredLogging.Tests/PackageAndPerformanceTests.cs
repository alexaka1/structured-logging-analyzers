...
public sealed class PackageAndPerformanceTests
{
    ...
    public void Packed_nupkg_has_analyzers_and_no_lib()
    {
        ...
        Assert.Contains(entries, e => e.Equals("analyzers/dotnet/cs/StructuredLogging.Analyzers.dll", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(entries, e => e.Equals("analyzers/dotnet/cs/StructuredLogging.CodeFixes.dll", StringComparison.OrdinalIgnoreCase));
    ...
    public void Sample_output_does_not_copy_analyzer_assemblies()
    {
        ...
        Assert.DoesNotContain("StructuredLogging.Analyzers.dll", files, StringComparer.OrdinalIgnoreCase);
        Assert.DoesNotContain("StructuredLogging.CodeFixes.dll", files, StringComparer.OrdinalIgnoreCase);
    }

    private static string Pack()
    {
        var repo = FindRepoRoot();
        var packProject = Path.Combine(repo, "pack", "StructuredLogging.Package", "Package.csproj");
        ...
        var nupkg = Directory.GetFiles(output, "StructuredLogging.Analyzers*.nupkg").Single();
        return nupkg;
    }
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir, "StructuredLogging.Roslyn.slnx")) ||
                File.Exists(Path.Combine(dir, "pack", "StructuredLogging.Package", "Package.csproj")))
            {
                return dir;
            }
